            ...
            <tbody>
                @foreach($clans as $clan)
                    <tr class="border-b border-gray-700 hover:bg-gray-800/70 transition-colors duration-200">
                        ...
                        <td class="px-4 py-4 text-center font-bold text-white"><a href="{{ route('clan.show', ['tag' => ltrim($clan['tag'], '#')]) }}" class="hover:underline">{{ $clan['name'] }}</a></td>
                        <td class="px-4 py-4 text-center">{{ $clan['members'] }}/50</td>
                        <td class="px-4 py-4 text-center text-yellow-400">{{ $clan['clanBuilderBasePoints'] }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
